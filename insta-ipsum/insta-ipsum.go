...
func pullText(paragraphs int) []byte {
	params := url.Values{}
	params.Set("paras", strconv.Itoa(paragraphs))
...
func GetIpsum(paragraphs int, clean_text bool) string {
	raw_text := pullText(paragraphs)
	return cleanText(raw_text, clean_text)
}

type IpsumHandler struct {}

func (i *IpsumHandler)

func (i *IpsumHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	io.Copy(w, )
